...
def Canny(dstImg):
    ...
    W, H = gray.shape
    # W,H-5
    paddle_gray = np.pad(gray,((4,4),(4,4)),'constant', constant_values=(0,0))
    new_gray = np.zeros([W, H])
    for i in range(W):
        for j in range(H):
                new_gray[i, j] = np.sum(paddle_gray[i:i + 5, j:j + 5] * gaussian)  # 与高斯矩阵卷积实现滤波
    # plt.imshow(new_gray, cmap="gray")
    
    #new_gray = cv.filter2D(gray,-1,gaussian)
...
if __name__ == '__main__':
    imgName = "C:\\Users\\yixiao\\Desktop\\NUTS\\pca_canny\\pca-canny-Canny\\072.png"
    # Canny
